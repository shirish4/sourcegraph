...
func TestPrepareWorkspace_Clone(t *testing.T) {
	...
	dir, err := handler.prepareWorkspace(context.Background(), runner, "torvalds/linux", "", "deadbeef", false, []string{})
	if err != nil {
	...
	expectedCommands := [][]string{
		...
		{"git", "-C", dir, "-c", "protocol.version=2", "-c", "http.extraHeader=Authorization: token-executor hunter2", "-c", "http.extraHeader=X-Sourcegraph-Actor-UID: internal", "fetch", "--progress", "--no-recurse-submodules", "origin", "deadbeef"},
...
func TestPrepareWorkspace_Clone_Subdirectory(t *testing.T) {
	...
	dir, err := handler.prepareWorkspace(context.Background(), runner, "torvalds/linux", "subdirectory", "deadbeef", false, []string{})
	if err != nil {
		t.Fatalf("unexpected error preparing workspace: %s", err)
	}
	defer os.RemoveAll(dir)
...
func TestPrepareWorkspace_ShallowClone(t *testing.T) {
	...
	dir, err := handler.prepareWorkspace(context.Background(), runner, "torvalds/linux", "", "deadbeef", true, []string{})
	if err != nil {
		t.Fatalf("unexpected error preparing workspace: %s", err)
	}
	defer os.RemoveAll(dir)
...
func TestPrepareWorkspace_SparseCheckout(t *testing.T) {
	...
	dir, err := handler.prepareWorkspace(context.Background(), runner, "torvalds/linux", "", "deadbeef", true, []string{"kernel"})
	if err != nil {
		t.Fatalf("unexpected error preparing workspace: %s", err)
	}
	defer os.RemoveAll(dir)
...
func TestPrepareWorkspace_NoRepository(t *testing.T) {
	...
	dir, err := handler.prepareWorkspace(context.Background(), runner, "", "", "", false, []string{})
	if err != nil {
		t.Fatalf("unexpected error preparing workspace: %s", err)
	}
	defer os.RemoveAll(dir)
